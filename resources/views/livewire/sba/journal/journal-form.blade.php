<div>
    <form wire:submit.prevent="submitJournal">
        ...
        <div class="mb-4">
            ...
            <select id="bible_story" wire:model="bible_story"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                required {{ $editMode ? '' : 'disabled' }}>
                <option value="">Select Season</option>
                @foreach ($this->courses as $course)
                    <option wire:key="{{ $course->id }}" value="{{ $course->id }}">S{{ $course->season_id }}
                        {{ $course->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <select id="lesson" wire:model="lesson"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                required {{ $editMode ? '' : 'disabled' }}>
        ...
        <div class="mb-4">
            ...
            1<input type="radio" id="difficulty1" wire:model="difficulty" value="1" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            2<input type="radio" id="difficulty2" wire:model="difficulty" value="2" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            3<input type="radio" id="difficulty3" wire:model="difficulty" value="3" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            4<input type="radio" id="difficulty4" wire:model="difficulty" value="4" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            5<input type="radio" id="difficulty5" wire:model="difficulty" value="5" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
        </div>
        ...
        <div class="mb-4">
            ...
            1<input type="radio" id="interest1" wire:model="interest" value="1" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            2<input type="radio" id="interest2" wire:model="interest" value="2" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            3<input type="radio" id="interest3" wire:model="interest" value="3" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            4<input type="radio" id="interest4" wire:model="interest" value="4" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
            5<input type="radio" id="interest5" wire:model="interest" value="5" class="ml-2 mr-2"
                {{ $editMode ? '' : 'disabled' }}>
        </div>

        <div class="mb-4">
            <label for="no_child" class="block text-sm font-medium text-gray-700">Number of Children</label>
            <input type="text" id="no_child" wire:model="no_child"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                required {{ $editMode ? '' : 'disabled' }}>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="no_salvation" wire:model="no_salvation"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                {{ $editMode ? '' : 'disabled' }}>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="improvement" wire:model="improvement"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                {{ $editMode ? '' : 'disabled' }}>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="textarea" id="feedback" wire:model="feedback"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                {{ $editMode ? '' : 'disabled' }}>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="textarea" id="testimony" wire:model="testimony"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                required {{ $editMode ? '' : 'disabled' }}>
        </div>
                    ...
                    <div class="flex flex-wrap gap-4 mt-2">
                        ...
                        @foreach ($pictures as $picture)
                            <div class="w-24 h-24 border rounded overflow-hidden flex items-center justify-center">
                                <img src="{{ is_string($picture) ? asset('storage/' . $picture) : $picture->temporaryUrl() }}"
                                    alt="Uploaded Picture" class="object-cover w-full h-full">
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        @else
            <div class="mb-4">
                <label for ="pictures" class="block text-gray-700">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                        viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 5v9m-5 0H5a1 1 0 0 0-1 1v4a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-4a1 1 0 0 0-1-1h-2M8 9l4-5 4 5m1 8h.01" />
                    </svg>Upload Pictures</label>

                <input type="file" id="pictures" wire:model="pictures" class="hidden" multiple
                    {{ $editMode ? '' : 'disabled' }}>
